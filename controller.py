...
class WikipediaController:
    ...
    def download_pdf(self):
        ...
        return {"path":TEMP_FILE,"fileName":self.keyword+".pdf"}
...
class GoogleDriveController:
    ...
    def upload(self,path,fileName="temp.pdf"):
        file_metadata = {'name': fileName}
        media = MediaFileUpload(path, mimetype='application/pdf')

        self.file = self.service.files().create(body=file_metadata,
                                           media_body=media,
                                           fields='id').execute()
